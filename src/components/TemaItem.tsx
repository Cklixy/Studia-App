  ...
  return (
    <div className={`apple-card p-3.5 sm:p-4 flex items-center justify-between gap-3 transition-all apple-tactile ${
      ...
      <div className="min-w-0 flex-1">
        <h4 className={`apple-headline truncate ${isCompleted ? 'line-through text-arctic-tertiary' : 'text-arctic-slate'}`}>
          {tema.nombre}
        </h4>
        {tema.tipo_contenido && (
      ...
      <button 
        ...
        disabled={loading}
        className="p-1.5 rounded-full hover:bg-black/[0.04] transition-colors disabled:opacity-50 apple-tactile shrink-0"
      >
        {isCompleted ? (
